<?php "Src/Templates/header.php";
?>

        <br>
        <div id="store">
          <div class="row">
            <div class="col-lg-3">
          
            
            
            </div>
            </div>

          
            </div>
          </div>
        </section>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/211.burger.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/208.broadways.png" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/209.festive.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/207.fanta.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/205.soda.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/202.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/201.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/200.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/14.mutura.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/13.popi.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/9.afia.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text">.</p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/7.club soda.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/6.tusker-cider.webp" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/6.Ribena-black-currant-juice.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/4.redlabel.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/.9fanta-spritzed-pineapple-can.png" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/209.festive.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/222.big g.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/223.mahamri.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/221.TropicalMints.webp" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/221.bigdaddy.webp" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div>
        <div class="card-group">
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/220.groundnut.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/202.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
          <div class="card">
            <img src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/219.blueband.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">Card title</h5>
              <p class="card-text"></p>
            </div>
            <div class="card-footer">
                <img  style="height: 20px; width: 20px;" src="file:///C:/Xammpo/htdocs/web_canteen/Assets/Images/2.cart.jpg " alt=""><a href="file:///C:/Xammpo/htdocs/web_canteen/Pages/contacts.html#"><button type="button"class= " btn btn-success"> Add to cart</button></a>
             
            </div>
          </div>
        </div></div>
        <nav aria-label="Page navigation example">
            <ul class="pagination">
              <li class="page-item">
                <a class="page-link" href="file:///C:/Xammpo/htdocs/web_canteen/Pages/Previous.html" aria-label="Previous">
                  <span aria-hidden="true">&laquo;</span>
                </a>
              </li>
              <li class="page-item"><a class="page-link" href="file:///C:/Xammpo/htdocs/web_canteen/Pages/1.html">1</a></li>
              <li class="page-item"><a class="page-link" href="file:///C:/Xammpo/htdocs/web_canteen/Pages/2.html">2</a></li>
              <li class="page-item"><a class="page-link" href="file:///C:/Xammpo/htdocs/web_canteen/Pages/3.html">3</a></li>
              <li class="page-item">
                <a class="page-link" href="file:///C:/Xammpo/htdocs/web_canteen/Pages/next.html" aria-label="Next">
                  <span aria-hidden="true">&raquo;</span>
                </a>
              </li>
            </ul>
          </nav>
        
        
        



          <?php 
          include "Src/Templates/Footer.php";
?>